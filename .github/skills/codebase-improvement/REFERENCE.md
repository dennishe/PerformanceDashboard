...
For this Swift codebase:

- Prefer boundary tests under `Tests/PerformanceDashboardTests/Services/`, `Tests/PerformanceDashboardTests/ViewModels/`, or `Tests/PerformanceDashboardTests/Shared/` based on ownership
- Validate build and tests with `swift test`
- For coverage: `swift test --enable-code-coverage && xcrun llvm-cov report .build/debug/PerformanceDashboardPackageTests.xctest/Contents/MacOS/PerformanceDashboardPackageTests -instr-profile .build/debug/codecov/default.profdata`

## RFC Template (Local Doc)
...
- Stepwise implementation order (interface, adapter wiring, migration, cleanup)
- Risk controls (feature flags, dual-path period, fallback behavior)
- Concrete file-level migration checklist

## Validation

- Exact new boundary tests to add
- Exact legacy tests to remove
- Commands run and outcomes (`swift test`, optional coverage script)
